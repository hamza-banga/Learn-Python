...
# open() returns a file object whose type depends on the mode, and through which the standard file operations such as reading and writing are performed. When open() is used to open a file in a text mode ('w', 'r', 'wt', 'rt', etc.), it returns a TextIOWrapper. When used to open
# a file in a binary mode, the returned class varies: in read binary mode, it returns a BufferedReader; in write binary and append binary modes, it returns a BufferedWriter, and in read/write mode, it returns a BufferedRandom.
...
numner = input("Enter The Number You Want To Add ")
# (function) input: (__prompt: Any = ...) -> str
...
# (function) writer: (csvfile: _Writer, dialect: _Dialect Like = ..., **fmtparams: Any) -> _writer
...
# writer.writerow([name ,  numner])
csv.writer(file).writerow([name , numner])
file.close()
